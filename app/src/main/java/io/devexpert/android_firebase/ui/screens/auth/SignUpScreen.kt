    ...
    if (email.isNotEmpty() && password.isNotEmpty()) {
        when (val result = auth.createUserWithEmailAndPassword(email, password)) {
            is AuthRes.Success -> {
                ...
                saveUserDataToFirestore(userData = UserData("","","","","","","","","")) // Guardar datos en Firestore
...
private fun saveUserDataToFirestore(userData: UserData) {
    val db = Firebase.firestore
    db.collection("users")
        .add(userData)
        .addOnSuccessListener { documentReference ->
            // Aquí puedes realizar acciones adicionales si la inserción es exitosa
